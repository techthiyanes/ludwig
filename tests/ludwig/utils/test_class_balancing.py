...
from ludwig.constants import NAME, OUTPUT_FLAG, PROC_COLUMN
...
def run_test_balance_data_ray(
    ...
    with ray_start(num_cpus=num_cpus, num_gpus=num_gpus):
        ...
        test_df = balance_data(input_df, input_features, preprocessing_parameters, backend)
...
def test_balance_data_ray(method, balance):
    ...
    run_test_balance_data_ray(df, features, preprocessing_params, target, balance)
...
def test_balance_data_local(method, balance):
    ...
    run_test_balance_data_local(df, features, preprocessing_params, target, balance, backend)
...
def test_non_binary_failure():
    ...
    with pytest.raises(ValueError):
        run_test_balance_data_local(df, features, preprocessing_params, target, 0.5, backend)
