...
class LinearLayer(torch.nn.Module):
    def __init__(self, in_features, out_features, use_bias=True, use_bn=False, **kwargs):
        ...
        self.linear = torch.nn.Linear(self.in_features, self.out_features, bias=self.use_bias and not self.use_bn)
        if self.use_bn:
...
class ScarfModel(LudwigModule):
    def __init__(
        ...
        self.projection_head = ProjectionHead(self.combiner.output_shape[-1], 256, 64)
    ...
    def _corrupt(self, inputs):
        ...
        for input_feature_name, input_values in inputs.items():
            ...
            augmentations[input_feature_name] = encoder.sample_augmentations(
                batch_size, self.training_set_metadata[input_feature_name]
            )
        ...
        corrupted_inputs = {
            input_feature_name: torch.from_numpy(np.where(mask[:, j], augmentations[input_feature_name], input_values))
...
class Pretrainer(Trainer):
    ...
    def pretrain(self, model: ECD, dataset: Dataset, **kwargs):
        _, train_stats, _, _ = self.train(self.ssl_model, training_set=dataset, **kwargs)
